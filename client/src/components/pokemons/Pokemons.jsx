...
export default function Pokemons() {
  ...
  const userFilter = useSelector(
    (state) => state.pokemonReducer.sortByCreatedBy
  );
  ...
  if (pokemons !== null && pokemons.length) {
    if (typeFilter === "todos") {
      ...
      pokemonsFiltered = pokemonsFiltered.filter((t) =>
        t.tipos.find((p) => p.name === typeFilter)
      );
    }
  }
  ...
  if (pokemonsFiltered.length === 0) {
    return (
      <div style={{ fontFamily: "Poppins" }}>
        <NavBar />
        <h3 style={{ marginTop: "5%", marginLeft: "2%", color: "black" }}>
          No se encuentran pokemons con el tipo seleccionado
        </h3>
      </div>
    );
  }
  if (pokemons !== null && pokemons.length === undefined) {
    ...
    return (
      <div>
        ...
        {pokemonsFiltered.length >= 9 || page > 9 ? (
          <div className={s.paginadoContainer}>
            <button
              className={s.paginadoButton}
              onClick={() => prevPage()}
            >{`<`}</button>
            <div>
              <h5
